function [stiff_nu] = stiff_nu_P2(geom,para,sln)
...
%% ---------------------------------------------------------------------------
    ...
    ls = para.ls; mu1 = para.mu1; mu2 = para.mu2; Ca = para.Ca;
...
%% ---------------------------------------------------------------------------
    ...
    for t = 1:cl2mem1-1
        mu = mu2;
        Ls21 = 1/Ca*tauelem_mem(t,1)*getStiffonRef1DP2P2der;
        Ls22 = 1/Ca*tauelem_mem(t,2)*getStiffonRef1DP2P2der;
        Ls23 = Ca*tauelem_mem(t,1)*getStiffonRef1DP2P2der;
        Ls24 = Ca*tauelem_mem(t,2)*getStiffonRef1DP2P2der;
        Ls6 = ls/mu/((pmem(t+1,1)-pmem(t,1))*lenelem(t))*getStiffonRef1DP2der;
        for ti = 1:3
            for tj = 1:3
                ind = ind + 1;
    ...
    stiff_nu.M21 = sparse(i21,j21,s21,2*Nmem+1,N+Ne);
    stiff_nu.M22 = sparse(i22,j22,s22,2*Nmem+1,N+Ne);
    
    stiff_nu.M23 = sparse(i23,j23,s23,2*Nmem+1,N+Ne);
    stiff_nu.M24 = sparse(i24,j24,s24,2*Nmem+1,N+Ne);
    
    stiff_nu.M6 = sparse(i6,j6,s6,2*Nmem+1,2*Nmem+1);
