function [p,t]=mydistmesh2d(fd,fh,h0,bbox,pfix,varargin)
...
dptol=.0005; ttol=.01; Fscale=1.2; deltat=.2; geps=.00001*h0; deps=sqrt(eps)*h0;
densityctrlfreq=20;
...
p=p(feval(fd,p,varargin{:})<-1e-18,:);                 % Keep only d<0 points
...
%     pause(0.01);
  ...
  if mod(itr,densityctrlfreq)==0 && any(L0>2*L)
      p(setdiff(reshape(bars(L0>1.8*L,:),[],1),1:nfix,'stable'),:)=[];
      N=size(p,1); pold=inf;
      continue;
  end
