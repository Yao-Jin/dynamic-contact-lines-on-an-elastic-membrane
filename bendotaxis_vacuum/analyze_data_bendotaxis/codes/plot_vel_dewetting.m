...
load('plotvel_dewetting_20_10_gap_4_xl_2_gamma2_0_3_cb_5e3_Ca_0_1_ind_14.mat');
...
% save('plotvel_dewetting_20_10_gap_4_xl_2_gamma2_0_3_cb_5e3_Ca_0_1_ind_9.mat','cnode','celem');
% load('plotvel_dewetting_20_10_gap_4_xl_2_gamma2_0_3_cb_5e3_Ca_0_1_ind_9.mat');
...
for k = 1:size(cnode,1)
    ...
    if ~updated
       if x<5
            for j = 1:Nffil-1
                if pffil(j,2)>y || pffil(j+1,2)<y
                    continue;
                end
                S = sqrt((x-pffil(j,1))^2+(y-pffil(j,2))^2)/sqrt((pffil(j+1,1)-pffil(j,1))^2+(pffil(j+1,2)-pffil(j,2))^2);
                weights = [vel(ffil2node(j:j+1),1:2); vel(ffil2edge(j)+N,1:2)];
                bases = [2*(S-1/2)*(S-1) 2*S*(S-1/2) -4*S*(S-1)];
                newvel(k,1:2) = bases*weights;
                updated = true;
                break;
       ...
       if x>=5
            for j = 1:Nffir-1
                if pffir(j,2)>y || pffir(j+1,2)<y
                    continue;
                end
                S = sqrt((x-pffir(j,1))^2+(y-pffir(j,2))^2)/sqrt((pffir(j+1,1)-pffir(j,1))^2+(pffir(j+1,2)-pffir(j,2))^2);
                weights = [vel(ffir2node(j:j+1),1:2); vel(ffir2edge(j)+N,1:2)];
                bases = [2*(S-1/2)*(S-1) 2*S*(S-1/2) -4*S*(S-1)];
                newvel(k,1:2) = bases*weights;
                updated = true;
                break;
...
set(gcf,'units','Normalized','position',[0.02,0.3,0.95,0.32]);
